...
def _update_nodata(filename: str) -> None:
    ...
    for t in Qgis.DataType:
        if (
            ...
            _debug(f"Set no data value to {stats.minimumValue}")
...
class RasterCalculator:
    ...
    def process(self, out_uri):
        manager = Manager()
        out = manager.dict()

        p = Process(target=_process, args=(self.project_uri, self.expression, out_uri, out))
        p.start()
        p.join()
